...
impl GitHub {
    ...
    pub fn new_workflow(&mut self, repo: &GithubRepoName, branch: &str) -> RunId {
        ...
        let workflow = WorkflowRun::new(run_id, repo.full_name(), branch);
...
impl Repo {
    ...
    pub fn new_workflow(&mut self, branch: Branch) {
        assert!(self.get_branch_by_name(&branch.name).is_some());
        self.workflow_runs
    }
    ...
    pub fn find_workflow(&self, id: RunId) -> Option<WorkflowRun> {
        self.workflow_runs
            .iter()
            .find(|w| w.workflow_run.run_id == id)
            .cloned()
    }
    ...
    pub fn find_workflows_by_check_suite_id(&self, id: CheckSuiteId) -> Vec<WorkflowRun> {
        self.workflow_runs
            .iter()
            .filter(|w| w.workflow_run.check_suite_id == id)
            .cloned()
            .collect()
    }
}
...
impl WorkflowRun {
    fn new(run_id: RunId, repo: GithubRepoName, branch: &Branch) -> Self {
        Self {
            status: WorkflowStatus::Pending,
            repository: repo,
            name: "Workflow1".to_string(),
            run_id,
            check_suite_id: CheckSuiteId(1), // TODO: set properly from GitHub
